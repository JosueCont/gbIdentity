...
const FormConfirmPassword = ({ isNewUser, title, isContainToken = "" }) => {
  ...
  const isValidatePassword = useSelector(
    (state) => state.authDuck.isValidatePassword
  );
  ...
  return (
    <View style={{ marginTop: 100 }}>
        ...
        <View style={{ marginBottom: 10 }}>
          ...
          {rules.map((item, index) => (
            <View style={{ marginBottom: 5 }} key={index}>
              <Text style={{ color: Colors.white, fontSize: getFontSize(13) }}>
                - {item?.message}
              </Text>
            </View>
          ))}
      ...
      <CustomButtom
        loading={loading}
        isDisabled={
          !(password != "" && repeatPassword != "" && isValidatePassword)
        }
        title={title}
        onPressed={() =>
          dispatch(
